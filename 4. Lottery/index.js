/*เรียกใช้ module : express ,request ,cheerio ,fs ,db เข้ามาในโปรเจค*/
var express = require('express'); 
var request = require('request');
var cheerio = require('cheerio');
var fs = require('fs');
var db = require('./db');
var app = express();  // สั่ง execute แล้วส่งค่าไว้ตัวแปร app
var count = 1;

//body-parser ตัวอ่านเเละส่งค่าของ body ได้จำเป็นต้องติดตั้ง //รับค่าเเบบ POST
const bodyParser = require('body-parser'); //นำ Module body-parser มาใช้ โดยอ้างอิงไว้ที่ตัวแปร bodyParser
/*คำสั่งของ app.use ดึงค่าที่อยู่ใน req.body ซึ่งจะเป็นชุดค่าที่ถูกส่งเข้ามา*/
// parse application/json : เรียกใช้งาน Method json()แล้วนำ Output ที่ได้ กำหนดเป็น Middleware ให้กับ Express ด้วย Method use()
...
app.post('/', (req,res) => {
        ...
        request('http://lottery.kapook.com/',function(err,res,body){
            ...
            if(!err && res.statusCode==200){
                ...
                $('.bigprize article .first-prize header ').filter(function(){
                    ...
                    prize = a[1]+" "+a[2];  //รางวัลที่ 1 
                    lottery = $(this).next().text(); //413494
                    a = $(this).next().next().text().split(" "); //รางวัลละ 6,000,000 
                    money = a[1]; //6,000,000
                    obj.push({prize:prize, lottery:lottery, money:money}); //เก็บค่าลงใน Array obj.
                });
    ...
    app.post('/search', function(req,res){
        var c=0,money,prize="";
        var a =[],j=[];
        a = req.body.id;  //การรับค่าเเบบ POST ไม่สามารถรับมาโดยตรงได้  เราจำเป็นต้อง รับค่ามาโดยใช้ req.body.ตัวเเปร; ซึ่งการใช้ body นั้นจำเป็นต้องติดตั้ง bodyParser ก่อน
        //check ว่า Id ที่รับเข้ามา เป็นอาร์เรย์ หรือ ไม่  เช่นรับมา เเค่ 1 id = 154782 มันจะไม่มองเป็นอาเรย์ เเต่มองเป็น String : Array.isArray(a)
        if(!Array.isArray(a)){
